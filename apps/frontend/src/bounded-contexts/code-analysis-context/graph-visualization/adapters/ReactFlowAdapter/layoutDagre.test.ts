...
describe('layoutDagre', () => {
  it('lays out connected nodes from left to right by default', () => {
    const nodes = [
      createNode({ id: 'source', data: { label: 'source', kind: 'file', fileName: 'source.ts', sourceOrigin: 'project' } }),
      createNode({ id: 'target', data: { label: 'target', kind: 'file', fileName: 'target.ts', sourceOrigin: 'project' } }),
    ]
